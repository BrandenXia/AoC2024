...
create function concat_number(l bigint, r bigint) returns bigint as
$$
select l * power(10, floor(log(r) + 1)) + r;
$$ language sql;
...
begin
    return query
        with recursive product(sequence, depth) as (select array []::text[], 0 as depth
                                                    ...
                                                    from product p,
                                                         (select '+' as op union all select '*' as op union all select '||' as op) o
    ...
    for i in 1..array_length(val_array, 1) - 1
        loop
            if operators[i] = '+' then
                ...
                result := concat_number(result, val_array[i + 1]);
            else
